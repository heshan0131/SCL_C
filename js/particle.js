...
DAT.Globe = function(container, colorFn) {
  ...
  function init() {
    ...
    mesh_globe = new THREE.Mesh(geometry, material);
    ...
    ///// particle material /////
    sprite1 = THREE.ImageUtils.loadTexture( imgDir+'circle.png' );
   
    material_particle = new THREE.SpriteMaterial( {
      map: sprite1,
      blending: THREE.AdditiveBlending,
      color: new THREE.Color( 0xffff00 )
    });

    console.log(material_particle);
    //material_particle = new THREE.ParticleSystemMaterial({size: 3, blending: THREE.AdditiveBlending, color: new THREE.Color( 0xff0000 )});
...
//        size = data[i + 2];
    ...
    for (i = 0; i < data.length; i++) {
      ...
      if (opts.name == 'particle'){
        addParticle(lat, lng, size);
        console.log("add 1 particle");
      }  else {
  ...
  function addParticle(lat, lng, size) {
    ...
    for ( var i = 0; i < 100; i++ ) {
      ...
      var particle = new THREE.Sprite( material_particle );
      console.log(material_particle);

      particle.position.x = 250 * Math.sin(phi) * Math.cos(theta);
      particle.position.y = 250 * Math.cos(phi);
      particle.position.z = 250 * Math.sin(phi) * Math.sin(theta);

      particle.scale.x = particle.scale.y = particle.scale.z = 1;
      //particle = new THREE.ParticleSystem( geometry, material_particle );

      //initParticle( particle, i * 10, lat, lng, size );
  ...
  function initParticle( particle, delay, lat, lng, size ) {
    ...
    //particle.position.x = 400 * Math.sin(phi) * Math.cos(theta);
    //particle.position.y = 400 * Math.cos(phi);
    //particle.position.z = 400 * Math.sin(phi) * Math.sin(theta);

    //particle.scale.x = particle.scale.y = Math.random() * 0.3 + 16;

    //particle.lookAt(mesh.position);
